...
def parse_topic_and_feedback(content: str) -> tuple[str, list[str]]:
    ...
    for line in lines:
        ...
        elif line.lower().startswith("user's request") or line.lower().startswith("用户修改建议"):
            continue
...
class Researcher(Role):
    ...
    async def _act(self) -> Message:
        ...
        if isinstance(msg.instruct_content, Report):
            ...
            if "topic" in msg.content:
                topic, feedbacks = parse_topic_and_feedback(msg.content)
            else:
                topic = msg.content
                feedbacks = []
        ...
        research_system_text = self.research_system_text(topic, feedback, todo)
    ...
    def research_system_text(self, topic: str, feedback: str, current_task: Action) -> str:
        """Generate task-specific system prompt"""
        base_prompt = get_research_system_text(topic, self.language,feedback)
