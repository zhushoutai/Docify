...
RESEARCH_BASE_SYSTEM = """You are an advanced AI research assistant. Your role is to critically analyze and synthesize information into comprehensive, objective, and well-structured research reports. You must uphold the highest standards of academic integrity, evidence-based reasoning, and methodological transparency."""
...
SEARCH_TOPIC_PROMPT = """Generate 2-3 optimal search queries for researching: "{topic}"
...
COLLECT_AND_RANKURLS_PROMPT = """Evaluate these {result_count} search results for query "{query}" about "{topic}":
Current Time: {time_stamp}
Results:
{results}

Filtering Criteria:
1. Remove irrelevant/off-topic results
2. Remove low-authority domains (except .gov/.edu/.org)
3. Prioritize recent content for time-sensitive topics

Output ranked indices as JSON array (e.g., [1,3,0])"""
...
CONDUCT_RESEARCH_PROMPT = """Based on the provided web search results, generate a rigorous, well-structured, and in-depth knowledge report on the topic: "{topic}":
Sources:
{content}
...
class CollectLinks(Action):
    ...
    async def run(
        ...
        system_text = system_text or RESEARCH_TOPIC_SYSTEM.format(topic=topic)
        
        # Generate initial queries
        queries = await self._generate_queries(topic, system_text)
        if not queries:
            queries = [topic]
    ...
    async def _generate_queries(self, topic: str, system_text: str) -> list[str]:
        """Generate optimized search queries"""
        try:
            response = await self._aask(SEARCH_TOPIC_PROMPT.format(topic=topic), [system_text])
...
def get_research_system_text(topic: str, language: str) -> str:
    """Generate system prompt for research tasks"""
    base = RESEARCH_TOPIC_SYSTEM.format(topic=topic)
